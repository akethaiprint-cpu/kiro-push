// @ts-nocheck
/**
 * Pricing Engine — Pure Calculation Logic (ported to TypeScript)
 * พอร์ตแบบ verbatim จาก js/pricing-engine.js เพื่อรักษา regression baseline ให้ตรงกับเว็บเดิม
 * โดเมนบริสุทธิ์: ไม่มี side effect / ไม่แตะ DOM / window / localStorage / network / Prisma
 *
 * NOTE: ใช้ `@ts-nocheck` ชั่วคราวสำหรับการพอร์ตรอบแรก (คงพฤติกรรมเดิม 1:1)
 * การเติม type ภายในแบบเต็มเป็นงานต่อเนื่อง — public boundary ถูก type ผ่าน index.ts + types.ts
 */
export const PricingEngine = {
  ...
  /**
   * Get active plate cost table (pure) — รับโหมดเป็น argument
   * Req 14.2, 14.3, 3.4, 3.5 — default 'legacy'; ส่ง 'datasheet2025' เพื่อใช้ตาราง datasheet
   * @param {'legacy'|'datasheet2025'} [mode]
   * @returns {{name:'legacy'|'datasheet2025', table:object}}
   */
  getActivePlateCostTable(mode) {
    // โดเมนบริสุทธิ์: รับโหมดเป็น argument แทนการอ่าน localStorage (Req 3.4, 3.5)
    // ไม่ระบุ → default 'legacy' (เทียบเท่าพฤติกรรมเดิมเมื่อไม่มีการสลับตาราง)
    if (mode === 'datasheet2025') {
      return { name: 'datasheet2025', table: this.PLATE_COST_DATASHEET2025 };
    }
  ...
  calculatePressSheet(productType, specs, priceTable) {
    ...
    const activePlateTable = this.getActivePlateCostTable(specs && specs.plateCostTableMode);
